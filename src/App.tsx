...
const EarnPage: React.FC = () => {
  const [tasks, setTasks] = useState<any[]>([]); // قائمة المهام
  const [loading, setLoading] = useState(true); // حالة التحميل
  const [userId, setUserId] = useState<number | null>(null); // user_id
  ...
  const fetchUserId = async () => {
    if (window.Telegram && window.Telegram.WebApp) {
      ...
      if (chatId) {
        ...
        console.error('No chat ID available from Telegram.');
      }
    } else {
      console.error('Telegram WebApp is not available.');
    }
  ...
  // جلب المهام اليومية بناءً على user_id
  const fetchTasks = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setTasks(data.tasks); // تحديث قائمة المهام
      } else {
  ...
  // تأثير جانبي لجلب user_id عند التحميل
  useEffect(() => {
    fetchUserId();
  }, []);

  // تأثير جانبي لجلب المهام بمجرد الحصول على user_id
  useEffect(() => {
  ...
  return (
    <div className="bg-black text-white h-screen flex flex-col items-center px-4">
      <h1 className="text-3xl font-bold mt-4">Earn Coins</h1>
      <p className="text-lg mb-6">Complete tasks to earn amazing rewards!</p>

      <div className="w-full max-w-md">
        {tasks.map((task) => (
          <div
            key={task.task_id}
            className={`flex justify-between items-center p-4 mb-4 rounded-lg shadow-lg ${
              task.completed ? 'bg-green-700' : 'bg-gray-800'
            }`}
          >
            <div>
              <h2 className="text-xl font-semibold">{task.task_name}</h2>
              <p className="text-sm">{task.task_points} points</p>
            </div>
            <button
